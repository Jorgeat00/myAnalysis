...
class myAnalysis(analysis):
	def init(self):  
		...
		self.CreateTH1F('cosa','nsuc canales',3,0,3)
		self.CreateTH1F('numjets','No de jets',5,0,5)
		
		self.CreateTH1F('masamuprueba','Masa (GeV)',20, 0.00, 300.00)
		self.CreateTH1F('seleccion','sucesos',4,0,4)
		...
		self.nlep=0
		self.pr=0
		self.leptones1=[] #prueba de cambiar de posición la definición de la lista
		
		'''
		self.CreateTTree('cuentas','cosa')
		self.lepTree=array('f',[0.])
		self.obj['cuentas'].Branch('muones',self.lepTree,'self.lepTree/F')
		'''
	def insideLoop(self,t):
		...
		'''
		if t.nJet==0:
			self.obj['numjets'].Fill(0)
		elif t.nJet==1:
			self.obj['numjets'].Fill(1)
		elif t.nJet==2:
			self.obj['numjets'].Fill(2)
		elif t.nJet==3:
			self.obj['numjets'].Fill(3)
		elif t.nJet==4:
			self.obj['numjets'].Fill(4)
		'''
		...
		# ~ for i in range(ncond):
			...
			# ~ for j in range(t.nElectron):
				...
				# ~ p.SetPtEtaPhiM(t.Electron_pt[j], t.Electron_eta[j], t.Electron_phi[j],t.Electron_mass[j])
				# ~ charge = t.Electron_charge[j]
				# ~ dxy = abs(t.Electron_dxy[j]) self.Muones
				# ~ dz  = abs(t.Electron_dz[j] )
		...
		# ~ for i in range(t.nMuon):
			...
			# ~ self.leptones1.append(fun.lepton(p, t.Muon_charge[i],13))
		...
		# ~ for j in range(t.nElectron): #no cuadra
			...
			# ~ self.leptones1.append(fun.lepton(v,t.Electron_charge[j],11))
			# ~ invmass = fun.InvMass(self.leptones[0], self.leptones[1]) if len(self.leptones) == 2 else 0
		...
		# ~ lepton1=self.leptones[0]
		# ~ lepton2=self.leptones[1]
		# ~ #if lepton1.charge*lepton2.charge >0: return
		# ~ if lepton1.GetPDGid() + lepton2.GetPDGid() ==26: #mumu
			# ~ self.obj['cosa'].Fill(0)
		# ~ elif lepton1.GetPDGid() + lepton2.GetPDGid() ==22: #ee
			# ~ self.obj['cosa'].Fill(1)
		# ~ elif lepton1.GetPDGid() + lepton2.GetPDGid() ==24: #emu
		...
		# ~ for i in range(t.nMuon):
			...
			# ~ if invmass <15: continue
			# ~ #if invmass < 105 and invmass >75: continue #en este caso no hay que poner el corte porque no hay Z
		...
		# ~ '''
		# ~ '''
		# ~ self.num=[t.nMuon,t.nElectron]
		# ~ for i in range(t.nMuon):
			# ~ if not self.num[0] ==1: continue
			# ~ #if not self.num[1] ==1: continue
			# ~ p=TLorentzVector()
			# ~ p.SetPtEtaPhiM(t.Muon_pt[i], t.Muon_eta[i], t.Muon_phi[i],t.Muon_mass[i])
			# ~ if p.Pt() < 20 or abs(p.Eta()) > 2.4: continue
			# ~ self.dos.append(fun.lepton(p, t.Muon_charge[i],13))
			...
			# ~ if invmass <15: continue
			# ~ if invmass < 105 and invmass >75: continue #pico del Z
			# ~ self.obj['masamuprueba'].Fill(invmass) 
			# ~ self.pr +=1
		# ~ '''
		# ~ '''
		# ~ self.leptones=[] 
		...
		# ~ for k in range(t.nGenDressedLepton): #esta variable no se usa para calcular los cortes, es solo para la aceptancia de la sección eficaz
			# ~ #son leptones que tienen unos cortes bastante fuertes en las variables
		...
		'''
		'''
		nLep = len(self.leptones1)
		if nLep < 2: return
		l0 = self.leptones1[0]
		l1 = self.leptones1[1]
		totId = l0.GetPDGid() + l1.GetPDGid()
		
		#self.lepTree.append(totId)
		#self.obj['cuentas'].Fill()
		
		if   totId == 24: 
			self.obj['seleccion'].Fill(1)
		elif totId == 22: 
			self.obj['seleccion'].Fill(2)
		elif totId == 26: 
			self.obj['seleccion'].Fill(3)
		'''
		'''
		# ~ #jets
		# ~ for k in range(2):  
